...
export function ColorPicker({
  ...
  const [color, setColor] = useState<Color>(() => {
    const hex = sanitizeHex(value || "#000000");
  ...
  }, [value]); // Removed color.hex dependency to avoid loop if parent updates value on change

  const updateColor = (newColor: Partial<Color>) => {
      setColor((prev) => {
        const next = { ...prev, ...newColor };
        
        // If updating HSL (from drag or slider), recalculate Hex
        if (newColor.h !== undefined || newColor.s !== undefined || newColor.l !== undefined) {
             const hex_formatted = hslToHex({ h: next.h, s: next.s, l: next.l });
             next.hex = hex_formatted;
             // Defer onChange to avoid side-effect in render phase
             setTimeout(() => onChange("#" + hex_formatted), 0);
        } else if (newColor.hex !== undefined) {
             // If updating Hex (from input), it's already set in next.hex
             setTimeout(() => onChange("#" + next.hex), 0);
        }
        return next;
      });
  };
  ...
  const handleHexInputChange = (newVal: string) => {
    ...
    // If valid hex length, update HSL
    if (hex.length === 6) {
      const hsl = hexToHsl({ hex });
      updateColor({ ...hsl, hex: hex });
    } else {
      // Just update local state for typing
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "flex w-full select-none flex-col items-center gap-3 overscroll-none rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900",
          className
        )}
      >
        ...
        <input
          ...
          max="360"
          value={color.h}
          className="h-3 w-full cursor-pointer appearance-none rounded-full border border-zinc-200 bg-white text-white placeholder:text-white dark:border-zinc-700"
          style={{
