...
import { DENTAL_MAX_WIDTH, DENTAL_FONT_HEADING } from "@/lib/dental-branding";
import { DentalNavbar, DentalFooter, DentalTopBar, DentalBackToTop } from "@/components/landing/dental";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";
import type { LandingSettings } from "@/lib/validations/landing-settings";
import { LandingThemeProvider } from "./landing-theme-provider";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { getColorForCategory } from "./dental-news";
  ...
  "tips-for-healthy-smile": {
    ...
    content: (
      <div className="space-y-6 text-slate-600 dark:text-slate-400 leading-relaxed text-lg">
        ...
        <div className="bg-rose-50 dark:bg-rose-900/10 p-8 rounded-3xl border border-rose-100 dark:border-rose-900/20 my-10">
           <p className="text-rose-600 dark:text-rose-400 font-bold italic text-xl">
           ...
           <p className="text-rose-500 font-black text-sm mt-4 uppercase tracking-widest">— Dr. Sarah Mitchell</p>
        </div>
...
export function BlogDetailContent({ branding, colors, dbPost, features, footer, contact, social }: BlogDetailContentProps) {
  ...
  if (dbPost) {
    ...
    post = {
      ...
      color: getColorForCategory(dbPost.category?.name ?? (dbPost.tags ? dbPost.tags.split(",")[0].trim() : "News")),
  ...
  if (!post) {
      return (
        <div className="dental-landing min-h-screen flex flex-col bg-white dark:bg-slate-950">
           <DentalTopBar features={features} colors={colors} />
           <DentalNavbar branding={branding} features={features} />
           <main className="flex-1 flex items-center justify-center pt-[8rem] pb-24">
              <div className="text-center space-y-6 px-4">
                 <h1 className="text-4xl md:text-5xl font-black text-rose-500">{t("postNotFound")}</h1>
                 <p className="text-slate-500 text-lg">{t("postNotFoundDesc")}</p>
                 <Link href="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-rose-500 text-white font-bold hover:bg-rose-600 transition-colors">
                    <ArrowLeft className="w-4 h-4" /> {t("backToHome")}
                 </Link>
              </div>
           </main>
           <DentalFooter branding={branding ?? undefined} footer={footer ?? undefined} contact={contact ?? undefined} social={social ?? undefined} />
        </div>
      );
  }
  ...
  return (
    <>
      <LandingThemeProvider colors={colors!} />
      <div className="dental-landing min-h-screen flex flex-col bg-white dark:bg-slate-950">
      <DentalTopBar features={features} colors={colors} />
      <DentalNavbar branding={branding} features={features} />
      <DentalBackToTop />
              ...
              <div className="max-w-3xl">
                ...
                <div
                  className={cn(
                    ...
                    !post.categoryColorHex && (post.color || "bg-rose-500")
                  )}
                ...
                {/* Title */}
                <h1 className={cn("text-3xl md:text-4xl lg:text-5xl font-black text-white leading-[1.1] mb-6", DENTAL_FONT_HEADING)}>
                  {post.title}
                </h1>
            ...
            <div className="mt-16 pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {(post.tags ? post.tags.split(",") : ["Dental Health", "Hygiene"]).map((tag: string) => (
                  <span
                    key={tag.trim()}
                    className="px-4 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 text-[10px] font-bold border border-slate-100 dark:border-slate-800"
                  >
                    {tag.trim()}
                  </span>
                ))}
