...
export function DentalMakeAppointment() {
  ...
  return (
    ...
    >
      ...
      <div className={cn("mx-auto relative z-10", DENTAL_MAX_WIDTH)}>
        <div className="bg-white dark:bg-[#0B0B1E] rounded-[32px] lg:rounded-[48px] overflow-hidden border border-slate-200/60 dark:border-slate-800/60">
          <div className="grid lg:grid-cols-12 items-stretch min-h-[480px]">
            ...
            <div className="lg:col-span-12 xl:col-span-7 p-6 sm:p-10 xl:p-12 text-slate-900 dark:text-white flex flex-col justify-center">
              ...
              >
                <h2 className={cn("text-2xl sm:text-3xl lg:text-4xl font-black mb-3 leading-tight tracking-tight text-slate-900 dark:text-white break-words", DENTAL_FONT_HEADING)}>
                ...
                <div className="flex flex-wrap gap-4 mb-8">
                   {BENEFITS.map((b, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                         <div className="w-4 h-4 rounded-full bg-rose-500/20 flex items-center justify-center text-rose-500 shrink-0">
              ...
              {isSuccess ? (
                <motion.div
                  ...
                  className="bg-emerald-50 dark:bg-emerald-500/10 border-2 border-emerald-500/20 rounded-2xl p-8 text-center"
                >
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.name && <span className="text-[9px] font-black text-rose-500 uppercase tracking-tighter">{errors.name}</span>}
                    </div>
                    <div className="relative group">
                      <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 group-focus-within:text-rose-500 transition-colors" />
                      <Input
                        id="apt-name"
                        ...
                        className={cn(
                          "h-11 pl-10 rounded-xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm placeholder:text-slate-300 dark:placeholder:text-slate-700 focus-visible:ring-rose-500/50 transition-all border-2",
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.phone && <span className="text-[9px] font-black text-rose-500 uppercase tracking-tighter">{errors.phone}</span>}
                    </div>
                    <div className="relative group">
                      <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 group-focus-within:text-rose-500 transition-colors" />
                      <Input
                        id="apt-phone"
                        type="tel"
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.service && <span className="text-[9px] font-black text-rose-500 uppercase tracking-tighter">{errors.service}</span>}
                    </div>
                    <div className="relative group">
                      <Stethoscope className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 z-10 group-focus-within:text-rose-500 transition-colors" />
                      <Select value={formData.service} onValueChange={(v) => setFormData({ ...formData, service: v })}>
                        <SelectTrigger
                          id="apt-service"
                          className={cn(
                            "h-11 pl-10 rounded-xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm focus:ring-rose-500/50 transition-all border-2",
                            errors.service && "border-rose-500/50"
                          )}
                        >
                          <SelectValue placeholder={t("selectServicePlaceholder")} />
                        </SelectTrigger>
                        <SelectContent className="rounded-xl border-slate-100 dark:border-slate-800 bg-white dark:bg-[#0B0B1E] text-slate-900 dark:text-white shadow-2xl p-1.5 min-w-[220px]">
                          {SERVICES.map((s) => (
                            <SelectItem 
                              key={s} 
                              value={s} 
                              className="hover:!bg-rose-500 hover:!text-white focus:!bg-rose-500 focus:!text-white cursor-pointer rounded-lg py-2.5 px-8 text-xs font-bold transition-colors m-0.5"
                            >
                              {s}
                            </SelectItem>
                          ))}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.date && <span className="text-[9px] font-black text-rose-500 uppercase tracking-tighter">{errors.date}</span>}
                    </div>
                    <div className="relative group">
                      <DatePicker
                        ...
                        className={cn(
                          "h-11 w-full rounded-xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm focus-visible:ring-rose-500/50 transition-all border-2 justify-start font-normal",
                  ...
                  <div className="sm:col-span-2 pt-6">
                    <LandingButton 
                      ...
                      className="w-full h-14 rounded-xl text-base font-black bg-rose-500 hover:bg-rose-600 border-none transition-all active:scale-[0.98] relative overflow-hidden group/btn"
                    >
            ...
            <div className="lg:col-span-5 relative hidden lg:block overflow-hidden bg-slate-900 border-l border-slate-100 dark:border-white/5">
               ...
               <motion.div
                 ...
                 className="absolute top-6 right-6 bg-white/10 backdrop-blur-2xl rounded-[24px] p-4 border border-white/20 max-w-[200px] shadow-xl z-20"
               >
                  <div className="flex items-center gap-3 mb-3">
                     <div className="w-9 h-9 rounded-xl bg-rose-500 flex items-center justify-center text-white shadow-lg shadow-rose-500/20">
