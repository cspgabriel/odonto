...
import { DENTAL_MAX_WIDTH, DENTAL_RADIUS_BUTTON } from "./config";
...
export function DentalNavbar({ showDoctors = false, branding, features }: { showDoctors?: boolean; branding?: LandingSettings["branding"]; features?: any }) {
  ...
  return (
    <div
      className={cn(
        ...
        "left-0 right-0 z-50 px-4 transition-all duration-500 ease-in-out",
        isScrolled 
          ? "top-4 pt-0" 
          : (features?.enableTopBar === false ? "top-4 pt-0" : "top-10 pt-2")
      )}
    >
      <div className={cn("mx-auto", DENTAL_MAX_WIDTH)}>
        <header
          className={cn(
            "flex h-20 items-center justify-between gap-4 px-6 transition-all duration-500 ease-in-out sm:px-4",
            "rounded-2xl border backdrop-blur-md",
            isScrolled
              ? "border-slate-200/60 bg-white/80 backdrop-blur-md shadow-sm dark:border-slate-800/60 dark:bg-slate-950/80"
        ...
        >
          ...
          >
            {branding?.primaryLogoUrl ? (
              ...
              <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/80 dark:from-primary dark:to-primary/60">
                {branding?.brandName || "Dental Clinic"}
              </span>
            )}
          </Link>
          {/* Desktop Navigation */}
          ...
          <div className="hidden lg:flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            <LandingButton size="default" variant="primary" className="bg-primary text-primary-foreground hover:bg-primary/90 dark:bg-primary dark:hover:bg-primary/90" asChild>
          ...
          <button
            ...
            className="lg:hidden flex items-center justify-center w-10 h-10 rounded-lg bg-slate-100/80 dark:bg-slate-800/80 text-slate-600 dark:text-white/80 hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors"
        ...
        <div
          className={cn(
            "lg:hidden fixed top-24 left-4 right-4 z-40 transition-all duration-300 ease-in-out rounded-2xl border backdrop-blur-md",
            mobileMenuOpen
          ...
          <nav className="flex flex-col p-4 max-h-[calc(100vh-8rem)] overflow-y-auto">
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ...
                      : "text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg"
                  )}
                >
                  {t(key)}
                </Link>
            ...
            {(showDarkToggle || showLangSwitcher) && (
              <div className="flex items-center gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800/50 px-2">
                {showDarkToggle && (
                  <LandingThemeSwitcher variant="outline" className="flex-1 justify-center rounded-xl bg-slate-50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-700/50" />
                )}
                {showLangSwitcher && (
                  <LandingLanguageSwitcher supportedLanguages={supportedLangs} variant="outline" className="flex-1 justify-center rounded-xl bg-slate-50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-700/50" />
                )}
              </div>
            )}
            ...
            <LandingButton 
              size="default" 
              variant="primary" 
              className="mt-4 w-full justify-center text-base py-6 bg-primary text-primary-foreground hover:bg-primary/90 dark:bg-primary dark:hover:bg-primary/90" 
