          ...
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {HIGHLIGHTS.map(({ num, title, desc }) => (
                ...
                >
                  <span className="text-sm font-bold text-rose-400">{num}</span>
                  <div>
                    <p className="font-semibold">{title}</p>
                    <p className="text-sm text-slate-300">{desc}</p>
                  </div>
                  <CheckCircle2 className="ml-auto h-5 w-5 shrink-0 text-rose-400" />
                </li>
              ))}
            </ul>
            <LandingButton
              size="lg"
              className="mt-6 rounded-full bg-gradient-to-r from-rose-500 to-rose-600 px-6 text-white shadow-md shadow-rose-500/25 hover:from-rose-600 hover:to-rose-700"
              asChild
            >
