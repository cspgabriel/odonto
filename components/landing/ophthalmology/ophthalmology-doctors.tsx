...
export function OphthalmologyDoctors() {
  return (
    <section id="doctors" className="border-b border-border bg-background py-12 sm:py-14">
      <div className="mx-auto max-w-7xl px-4">
        <p className="text-center text-xs font-semibold uppercase tracking-widest text-teal-500">
          Our Team
        </p>
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {DOCTORS.map(({ name, specialty, image }) => (
            <Card
              key={name}
              className="group overflow-hidden rounded-3xl border-slate-100 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:ring-2 hover:ring-teal-100"
            >
              <CardContent className="p-0">
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-t-2xl bg-slate-100 ring-2 ring-teal-100 ring-offset-2 transition-colors group-hover:ring-teal-200">
                  <Image
                    src={image}
                    alt={name}
                    fill
