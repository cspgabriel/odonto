...
export function OphthalmologyTestimonials({ testimonials }: { testimonials?: ContentSettings["testimonials"] }) {
    ...
    >
      <div className={cn("mx-auto px-4 relative z-10", OPHTHALMOLOGY_MAX_WIDTH)}>
        ...
        <div className="flex flex-col items-center text-center mb-12 md:mb-16 px-4">
           ...
           <h2 className={cn("text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 dark:text-white leading-[1.1] mb-4 md:mb-6 break-words", OPHTHALMOLOGY_FONT_HEADING)}>
        ...
        <div className="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2 lg:grid-cols-3">
          {TESTIMONIALS.map((t, index) => (
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 ring-slate-900/10 dark:ring-white/10 rounded-3xl md:rounded-3xl border border-transparent p-5 md:p-6 ring-1 transition-all duration-300 hover:ring-teal-500/30 hover:shadow-2xl hover:shadow-teal-500/10 group cursor-pointer"
            >
              ...
              <div className="flex items-center gap-3">
                <Avatar className="ring-slate-900/5 dark:ring-white/10 size-10 border border-transparent shadow shadow-slate-200/50 dark:shadow-none ring-1 bg-slate-50 dark:bg-slate-800">
                  ...
                  <AvatarFallback className="font-bold text-xs bg-teal-50 text-teal-500">{t.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <div>
                  <div className="text-slate-900 dark:text-white text-[13px] font-black uppercase tracking-tight">{t.name}</div>
                  <div className="flex items-center gap-2">
                    <span className="text-teal-500 text-[10px] font-black uppercase tracking-widest">{t.role}</span>
                  </div>
                </div>
              </div>
            </motion.div>
