...
export function AppointmentHero() {
  ...
  return (
    <section className="relative overflow-hidden pt-36 pb-24 lg:pt-52 lg:pb-48 bg-[#FFF2F6] dark:bg-slate-900/50 px-4">
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/10 dark:bg-primary/20 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3 -z-10"></div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-24">
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl">
             <motion.div 
               ...
               className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 dark:bg-primary/20 text-primary text-sm font-black uppercase tracking-[0.2em] mb-8"
             >
             ...
             <motion.h1 
               ...
               animate={h1Animate}
               transition={h1Trans}
               className={cn("text-5xl sm:text-7xl lg:text-8xl font-black text-[#2D2D5F] dark:text-white leading-[1.05] mb-10 break-words", GENERAL_FONT_HEADING)}
             >
               {t("title")}
             </motion.h1>
             ...
             >
               <Link href="/" className="flex items-center gap-2 text-primary font-black hover:text-primary/90 transition-colors bg-white/80 dark:bg-slate-800/80 px-4 py-2 rounded-2xl shadow-sm">
          ...
          <div className="flex-1 relative w-full max-w-[500px]">
             <motion.div 
               initial={reduced ? {} : { opacity: 0, scale: 0.8, rotate: 5 }}
               animate={reduced ? {} : { opacity: 1, scale: 1, rotate: 0 }}
               transition={reduced ? { duration: 0 } : { delay: 0.2, duration: 1, ease: "easeOut" }}
               className="relative"
             >
                ...
                <motion.div 
                   ...
                   className="absolute -top-6 -left-6 w-28 h-28 bg-white dark:bg-slate-800 rounded-[32px] shadow-2xl flex items-center justify-center p-3 border border-slate-50 dark:border-slate-700"
                >
                   <Image src={LANDING_IMAGES.doctor2} alt="Icon" width={100} height={100} className="rounded-xl object-cover h-full" />
                </motion.div>

                {/* Decorative Circle Dot */}
                <div className="absolute -bottom-4 right-1/4 w-8 h-8 bg-primary rounded-full animate-ping opacity-20"></div>
             </motion.div>
          </div>
