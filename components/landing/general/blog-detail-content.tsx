...
import { GENERAL_MAX_WIDTH, GENERAL_FONT_HEADING } from "./config";
import { GeneralNavbar, GeneralFooter, GeneralTopBar, GeneralBackToTop } from "@/components/landing/general";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";
import type { LandingSettings } from "@/lib/validations/landing-settings";
import { GeneralThemeProvider } from "./general-theme-provider";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import { getColorForCategory } from "./general-news";
  ...
  "tips-for-healthy-smile": {
    ...
    content: (
      <div className="space-y-6 text-slate-600 dark:text-slate-400 leading-relaxed text-lg">
        ...
        <div className="bg-primary/10 dark:bg-primary/10 p-8 rounded-3xl border border-primary/20 dark:border-primary/20 my-10">
           ...
           <p className="text-primary font-black text-sm mt-4 uppercase tracking-widest">— Dr. Sarah Mitchell</p>
        </div>
...
export function BlogDetailContent({ branding, colors, dbPost, features, footer, contact, social }: BlogDetailContentProps) {
  ...
  if (!post) {
      return (
        <div className="general-landing min-h-screen flex flex-col bg-white dark:bg-slate-950">
           <GeneralTopBar features={features} colors={colors} />
           <GeneralNavbar branding={branding} features={features} />
           <main className="flex-1 flex items-center justify-center pt-[8rem] pb-24">
              <div className="text-center space-y-6 px-4">
                 <h1 className="text-4xl md:text-5xl font-black text-primary">{t("postNotFound")}</h1>
                 <p className="text-slate-500 text-lg">{t("postNotFoundDesc")}</p>
                 <Link href="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-primary text-white font-bold hover:bg-primary/90 transition-colors">
                    <ArrowLeft className="w-4 h-4" /> {t("backToHome")}
                 </Link>
              </div>
           </main>
           <GeneralFooter branding={branding ?? undefined} footer={footer ?? undefined} contact={contact ?? undefined} social={social ?? undefined} />
        </div>
      );
  }
  ...
  return (
    <>
      <GeneralThemeProvider colors={colors!} />
      <div className="general-landing min-h-screen flex flex-col bg-white dark:bg-slate-950">
      <GeneralTopBar features={features} colors={colors} />
      <GeneralNavbar branding={branding} features={features} />
      <GeneralBackToTop />
              ...
              <div className="max-w-3xl">
                ...
                <div
                  className={cn(
                    ...
                    !post.categoryColorHex && (post.color || "bg-primary")
                  )}
                ...
                {/* Title */}
                <h1 className={cn("text-3xl md:text-4xl lg:text-5xl font-black text-white leading-[1.1] mb-6", GENERAL_FONT_HEADING)}>
                  {post.title}
                </h1>
          ...
          <div>
            ...
            <div className="mt-16 pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {(post.tags ? post.tags.split(",") : ["Clinic", "Health"]).map((tag: string) => (
                  <span
                    key={tag.trim()}
