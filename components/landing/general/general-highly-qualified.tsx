...
import { GENERAL_MAX_WIDTH, GENERAL_SECTION_TITLE, GENERAL_SECTION_DESCRIPTION, GENERAL_SECTION_PADDING, GENERAL_SECTION_BORDER, GENERAL_SECTION_BG, GENERAL_FONT_HEADING } from "./config";
...
export function GeneralHighlyQualified() {
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        "absolute -z-10 bg-primary/10 dark:bg-primary/10 blur-[120px] rounded-full",
      ...
      <div className={cn("mx-auto px-4 sm:px-6 relative z-10", GENERAL_MAX_WIDTH)}>
        <div className="grid gap-8 sm:gap-10 lg:gap-12 lg:grid-cols-12 lg:items-start">
          ...
          {isRTL && (
            <div className="lg:col-span-7 lg:pr-6">
              ...
              <motion.p 
                ...
                className={cn("mt-4 sm:mt-6 mb-6 sm:mb-8 lg:mb-10", GENERAL_SECTION_DESCRIPTION)}
              >
                {t("description")}
              </motion.p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {DOCTORS.map(({ name, specialty, image }, idx) => (
                  <motion.div
                    ...
                    className="group relative bg-white dark:bg-slate-800 rounded-3xl sm:rounded-[32px] p-4 sm:p-5 border border-slate-100 dark:border-slate-700/50 transition-all duration-300 transform hover:-translate-y-2 cursor-pointer"
                  >
                    <div className="flex items-center gap-4 sm:gap-5 flex-row-reverse">
                      <div className="relative w-20 h-20 sm:w-24 sm:h-24 shrink-0 overflow-hidden rounded-2xl sm:rounded-3xl bg-slate-100 dark:bg-slate-700">
                        <Image
                          src={image}
                      ...
                      <div className="flex-1 text-right">
                        <h4 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white group-hover:text-primary transition-colors break-words">{name}</h4>
                        ...
                        <div className="flex items-center gap-2 flex-row-reverse justify-end">
                           {SOCIAL.slice(0, 3).map(({ Icon, label, hoverColor }) => (
                             <button key={label} className={cn(
                               "w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 transition-all duration-300 cursor-pointer",
                               "hover:text-white",
                               hoverColor
                             )}>
                    ...
                    <div className="absolute top-3 sm:top-4 left-3 sm:left-4 opacity-0 group-hover:opacity-100 transition-opacity">
                       <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center text-primary">
              ...
              >
                 <Link href="/doctors" className="flex items-center gap-2 sm:gap-3 font-black text-slate-900 dark:text-white group cursor-pointer flex-row-reverse">
                    <span className="text-sm sm:text-base border-b-2 border-slate-200 group-hover:border-primary transition-colors">{t("viewAllDoctors")}</span>
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-all group-hover:rotate-180">
          ...
          <div className="relative group lg:col-span-5">
            <motion.div 
               ...
               className="relative"
            >
              <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl sm:rounded-[32px] lg:rounded-[40px] bg-white dark:bg-slate-800 border-4 sm:border-6 lg:border-8 border-white dark:border-slate-800 cursor-pointer">
                <Image
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-transparent p-4 sm:p-6 lg:p-8 pt-12 sm:pt-16 lg:pt-20 transition-all group-hover:pb-6 sm:group-hover:pb-8 lg:group-hover:pb-10">
                   <div className={cn("flex items-center gap-2 mb-2", isRTL && "flex-row-reverse")}>
                      <div className="bg-primary w-2 h-2 rounded-full animate-ping"></div>
                      <span className="text-primary text-xs font-bold uppercase tracking-widest">{t("availableToday")}</span>
                   </div>
              ...
              <motion.div 
                 ...
                 className={cn(
                   "absolute -top-4 sm:-top-6 bg-white dark:bg-slate-800 rounded-3xl sm:rounded-[32px] p-4 sm:p-6 border-2 sm:border-4 border-primary/10 dark:border-slate-700 cursor-pointer",
                   isRTL ? "-left-4 sm:-left-6 sm:max-w-[180px]" : "-right-4 sm:-right-6 sm:max-w-[200px]"
                 )}
              >
                 <div className="flex flex-col items-center text-center">
                    <div className="w-10 h-10 sm:w-14 sm:h-14 bg-primary/10 dark:bg-primary/20 rounded-full flex items-center justify-center mb-2 sm:mb-3">
                       <UserCheck className="w-5 h-5 sm:w-7 sm:h-7 text-primary" />
                    </div>
            ...
            <div className="lg:col-span-7 lg:pl-6">
              ...
              >
                 <Link href="/doctors" className="flex items-center gap-2 sm:gap-3 font-black text-slate-900 dark:text-white group cursor-pointer">
                    <span className="text-sm sm:text-base border-b-2 border-slate-200 group-hover:border-primary transition-colors">{t("viewAllDoctors")}</span>
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-slate-200 dark:border-slate-700 flex items-center justify-center group-hover:bg-primary group-hover:text-primary-foreground transition-all">
