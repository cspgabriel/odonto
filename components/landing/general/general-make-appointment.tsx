...
import { GENERAL_MAX_WIDTH, GENERAL_SECTION_TITLE, GENERAL_SECTION_DESCRIPTION, GENERAL_SECTION_PADDING, GENERAL_SECTION_BG, GENERAL_TITLE_DESCRIPTION_GAP, GENERAL_HEADER_CONTENT_GAP } from "./config";
import { VerticalCutReveal } from "@/components/ui/vertical-cut-reveal";
import { useReducedMotion } from "@/lib/hooks/use-reduced-motion";

export function GeneralMakeAppointment() {
  ...
  return (
    <section
      id="appointment"
      ref={containerRef}
      className={cn("relative overflow-hidden px-4", GENERAL_SECTION_BG, GENERAL_SECTION_PADDING)}
    >
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10 w-[800px] h-[800px] bg-primary/10 dark:bg-primary/10 blur-[150px] rounded-full"></div>
      
      <div className={cn("mx-auto relative z-10", GENERAL_MAX_WIDTH)}>
        <div className="bg-white dark:bg-[#0B0B1E] rounded-3xl lg:rounded-[48px] overflow-hidden border border-slate-200/60 dark:border-slate-800/60">
          <div className="grid lg:grid-cols-12 items-stretch min-h-[480px]">
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.name && <span className="text-[9px] font-black text-primary uppercase tracking-tighter">{errors.name}</span>}
                    </div>
                    <div className="relative group">
                      <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 group-focus-within:text-primary transition-colors" />
                      <Input
                        id="apt-name"
                        ...
                        className={cn(
                          "h-11 pl-10 rounded-2xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm placeholder:text-slate-300 dark:placeholder:text-slate-700 focus-visible:ring-primary/50 transition-all border-2",
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.phone && <span className="text-[9px] font-black text-primary uppercase tracking-tighter">{errors.phone}</span>}
                    </div>
                    <div className="relative group">
                      <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 group-focus-within:text-primary transition-colors" />
                      <Input
                        id="apt-phone"
                        type="tel"
                        ...
                        className={cn(
                          "h-11 pl-10 rounded-2xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm placeholder:text-slate-300 dark:placeholder:text-slate-700 focus-visible:ring-primary/50 transition-all border-2",
                          errors.phone && "border-primary/50"
                        )}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.service && <span className="text-[9px] font-black text-primary uppercase tracking-tighter">{errors.service}</span>}
                    </div>
                    <div className="relative group">
                      <Stethoscope className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500 z-10 group-focus-within:text-primary transition-colors" />
                      <Select value={formData.service} onValueChange={(v) => setFormData({ ...formData, service: v })}>
                        <SelectTrigger
                          id="apt-service"
                          className={cn(
                            "h-11 pl-10 rounded-2xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm focus:ring-primary/50 transition-all border-2",
                            errors.service && "border-primary/50"
                          )}
                        >
                        ...
                        <SelectContent className="rounded-2xl border-slate-100 dark:border-slate-800 bg-white dark:bg-[#0B0B1E] text-slate-900 dark:text-white shadow-2xl p-1.5 min-w-[220px]">
                          {SERVICES.map((s) => (
                            <SelectItem 
                              key={s} 
                              value={s} 
                              className="hover:!bg-primary hover:!text-primary-foreground focus:!bg-primary focus:!text-primary-foreground cursor-pointer rounded-xl py-2.5 px-8 text-xs font-bold transition-colors m-0.5"
                            >
                              {s}
                            </SelectItem>
                          ))}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.date && <span className="text-[9px] font-black text-primary uppercase tracking-tighter">{errors.date}</span>}
                    </div>
                    <div className="relative group">
                      <DatePicker
                        ...
                        className={cn(
                          "h-11 w-full rounded-2xl border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-sm focus-visible:ring-primary/50 transition-all border-2 justify-start font-normal",
                          errors.date && "border-primary/50",
                  ...
                  <div className="sm:col-span-2 pt-6">
                    <LandingButton 
                      ...
                      className="w-full h-14 rounded-xl text-base font-black text-white border-none transition-all active:scale-[0.98] relative overflow-hidden group/btn bg-gradient-to-r bg-primary hover:bg-primary/90 dark:bg-primary dark:hover:bg-primary/90 text-primary-foreground"
                    >
            ...
            {/* Right: Immersive Image with Top-Right Badge */}
            <div className="lg:col-span-5 relative hidden lg:block overflow-hidden bg-slate-900 border-l border-slate-100 dark:border-white/5">
               <Image
                 src={LANDING_IMAGES.makeAppointmentDoctors}
               ...
               <motion.div
                 ...
                 className="absolute top-6 right-6 bg-white/10 backdrop-blur-2xl rounded-3xl p-4 border border-white/20 max-w-[200px] shadow-xl z-20"
               >
