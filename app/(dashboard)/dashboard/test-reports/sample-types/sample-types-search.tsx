...
export function SampleTypesSearch({
  defaultValue = "",
  pageSize = 10,
  status = "all",
  createAction,
  ...
  const updateQuery = useCallback(
    (updates: { q?: string; status?: string }) => {
      ...
      router.push(`/dashboard/test-reports/sample-types?${params.toString()}`);
    },
    ...
    >
      <div className="flex flex-col sm:flex-row items-center w-full gap-3">
        <div className="relative flex-1 w-full min-w-0">
          <Label htmlFor="sample-search" className="sr-only">{t("searchSampleTypesLabel")}</Label>
          ...
          <Input
            id="sample-search"
            placeholder={t("searchSampleTypesPlaceholder")}
            ...
            <div className="flex items-center gap-1 shrink-0 ml-1">
              {selectedCount > 0 && onDeleteSelected && (
                <AlertDialog>
                  ...
                  <AlertDialogContent className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0B1E]">
                    <AlertDialogHeader>
                      <AlertDialogTitle className="font-heading">{t("bulkDeleteSampleTypesTitle")}</AlertDialogTitle>
                      <AlertDialogDescription className="text-slate-500">
                        {t("bulkDeleteSampleTypesDescription", { count: selectedCount })}
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  setValue("");
                  onClearSelection?.();
                  router.push(`/dashboard/test-reports/sample-types?page=1&pageSize=${pageSize}`);
                }}
              >
                {t("clear")}
              </Button>
