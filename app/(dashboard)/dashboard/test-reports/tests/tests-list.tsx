...
import { deleteLaboratoryTest, bulkDeleteLaboratoryTests } from "@/lib/actions/lab-test-actions";
...
export type TestWithRelations = {
  ...
  turnaroundTimeName: string | null;
};
...
export function TestsList({
  ...
  const [editData, setEditData] = useState<TestWithRelations | null>(null);
  ...
  const toggleAll = (checked: boolean) => {
    setSelectedRows(checked ? new Set(list.map((t) => t.id)) : new Set());
  };

  async function handleDelete(id: string, name: string) {
    const result = await deleteLaboratoryTest(id);
    if (result.success) {
  ...
  const searchWithProps = searchContent && React.isValidElement(searchContent)
    ? React.cloneElement(searchContent as React.ReactElement<{ selectedCount?: number; onDeleteSelected?: () => void; onClearSelection?: () => void }>, {
        ...
        onDeleteSelected: async () => {
          ...
          const result = await bulkDeleteLaboratoryTests(ids);
          if (result.success) {
            setSelectedRows(new Set());
            toast.success(t("testsDeletedCount", { count: ids.length }));
            router.refresh();
          } else {
            toast.error(result.error);
          }
        ...
        <TableBody>
          {list.map((row) => (
            <TableRow key={row.id} className="border-slate-200/60 dark:border-slate-800/60 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group">
              ...
              <TableCell className="py-4">{row.sampleTypeName ?? "—"}</TableCell>
              <TableCell className="py-4">{row.methodologyName ?? "—"}</TableCell>
              <TableCell className="py-4">{row.turnaroundTimeName ?? "—"}</TableCell>
              <TableCell className="py-4">{row.price ?? "—"}</TableCell>
                ...
                <div className="flex justify-end gap-1">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0B1E]">
                      <AlertDialogHeader>
                        <AlertDialogTitle className="font-heading text-lg font-bold">{t("deleteTestTitle")}</AlertDialogTitle>
                        <AlertDialogDescription className="text-slate-500 text-sm">
                          {t("deleteTestDescription", { name: row.name })}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
      ...
      <Sheet open={!!editId} onOpenChange={(o) => { if (!o) { setEditId(null); setEditData(null); } }}>
        <SheetContent side="right" className="w-full sm:w-[600px] sm:max-w-[600px] border-l border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E] flex flex-col p-0 overflow-hidden shadow-2xl">
          <SheetHeader className="p-6 pb-4 border-b border-slate-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
            <SheetTitle className="text-xl font-black tracking-tight font-heading text-slate-900 dark:text-white">{t("editTest")}</SheetTitle>
          </SheetHeader>
