...
export function TurnaroundTimesSearch({
  defaultValue = "",
  pageSize = 10,
  status = "all",
  createAction,
  ...
  const updateQuery = useCallback(
    (updates: { q?: string; status?: string }) => {
      ...
      router.push(`/dashboard/test-reports/turnaround-times?${params.toString()}`);
    },
    ...
    >
      <div className="flex flex-col sm:flex-row items-center w-full gap-3">
        <div className="relative flex-1 w-full min-w-0">
          <Label htmlFor="turnaround-search" className="sr-only">{t("searchTurnaroundTimesLabel")}</Label>
          ...
          <Input
            id="turnaround-search"
            placeholder={t("searchTurnaroundTimesPlaceholder")}
        ...
        <div className="flex w-full sm:w-auto shrink-0 items-center gap-2">
          <Select value={searchParams.get("status") ?? "all"} onValueChange={(v) => updateQuery({ status: v })}>
            ...
            <SelectContent className="rounded-xl border-slate-200 dark:border-slate-800">
              ...
              <SelectItem value="active" className="cursor-pointer font-medium text-xs">{t("active")}</SelectItem>
              <SelectItem value="inactive" className="cursor-pointer font-medium text-xs">{t("inactive")}</SelectItem>
            </SelectContent>
          </Select>
          {hasActiveFilters && (
            <div className="flex items-center gap-1 shrink-0 ml-1">
              {selectedCount > 0 && onDeleteSelected && (
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button type="button" variant="ghost" className="h-10 px-3 rounded-lg font-bold text-rose-500 hover:text-rose-600 hover:bg-rose-50 transition-all flex items-center gap-1.5">
                  ...
                  <AlertDialogContent className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0B1E]">
                    <AlertDialogHeader>
                      <AlertDialogTitle className="font-heading">{t("bulkDeleteTurnaroundTimesTitle")}</AlertDialogTitle>
                      <AlertDialogDescription className="text-slate-500">
                        {t("bulkDeleteTurnaroundTimesDescription", { count: selectedCount })}
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  setValue("");
                  onClearSelection?.();
                  router.push(`/dashboard/test-reports/turnaround-times?page=1&pageSize=${pageSize}`);
                }}
              >
                {t("clear")}
              </Button>
