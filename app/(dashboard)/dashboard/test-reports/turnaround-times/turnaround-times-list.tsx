...
import { deleteTurnaroundTime, bulkDeleteTurnaroundTimes } from "@/lib/actions/lab-test-actions";
import { toast } from "sonner";
import { TurnaroundTimeForm } from "./turnaround-time-form";

export type TurnaroundTimeWithCategory = {
  id: string;
  name: string;
  code: string | null;
  priority: string;
...
export function TurnaroundTimesList({
  times: list,
  searchContent,
  createAction,
  children,
}: {
  times: TurnaroundTimeWithCategory[];
  ...
  const [editData, setEditData] = useState<TurnaroundTimeWithCategory | null>(null);
  ...
  function handleDelete(id: string, name: string) {
    const promise = deleteTurnaroundTime(id).then((r) => {
      if (!r.success) throw new Error(r.error);
      return r;
    });
    toast.promise(promise, {
      loading: t("deletingTurnaroundTime"),
      success: () => {
        router.refresh();
        return t("turnaroundTimeDeleted");
      },
      error: (err) => (err instanceof Error ? err.message : t("failedToDeleteTurnaroundTime")),
    });
  }
    ...
    return (
      <div className="overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm bg-white dark:bg-[#0B0B1E]">
        ...
        <EmptyState
          ...
          title={t("noTurnaroundTimesFound")}
          description={t("noTurnaroundTimesDescription")}
          action={createAction}
        />
      </div>
    );
  ...
  const searchWithProps = searchContent && React.isValidElement(searchContent)
    ? React.cloneElement(searchContent as React.ReactElement<{ selectedCount?: number; onDeleteSelected?: () => void; onClearSelection?: () => void }>, {
        ...
        onDeleteSelected: async () => {
          ...
          const promise = bulkDeleteTurnaroundTimes(ids).then((r) => {
            if (!r.success) throw new Error(r.error);
            return r;
          });
          toast.promise(promise, {
            loading: t("deletingTurnaroundTimesCount", { count: ids.length }),
            success: () => {
              setSelectedRows(new Set());
              router.refresh();
              return t("turnaroundTimesDeletedCount", { count: ids.length });
            },
            error: (err) => (err instanceof Error ? err.message : t("failedToDeleteTurnaroundTime")),
          });
        },
        ...
        <TableBody>
          {list.map((row) => (
            <TableRow key={row.id} className="border-slate-200/60 dark:border-slate-800/60 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group">
              ...
              <TableCell className="py-4">{row.priority}</TableCell>
              <TableCell className="py-4">{row.duration}</TableCell>
              <TableCell className="py-4">{row.categoryName ?? "—"}</TableCell>
                ...
                <div className="flex justify-end gap-1">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B0B1E]">
                      <AlertDialogHeader>
                        <AlertDialogTitle className="font-heading text-lg font-bold">{t("deleteTurnaroundTimeTitle")}</AlertDialogTitle>
                        <AlertDialogDescription className="text-slate-500 text-sm">
                          {t("deleteTurnaroundTimeDescription", { name: row.name })}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
      ...
      <Sheet open={!!editId} onOpenChange={(o) => { if (!o) { setEditId(null); setEditData(null); } }}>
        <SheetContent side="right" className="w-full sm:w-[600px] sm:max-w-[600px] border-l border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E] flex flex-col p-0 overflow-hidden shadow-2xl">
          <SheetHeader className="p-6 pb-4 border-b border-slate-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
            <SheetTitle className="text-xl font-black tracking-tight font-heading text-slate-900 dark:text-white">{t("editTurnaroundTime")}</SheetTitle>
          </SheetHeader>
