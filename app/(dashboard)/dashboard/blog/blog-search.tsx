...
export function BlogSearch({
  ...
  const hasFilters = searchParams.get("q") || searchParams.get("status") || selectedCount > 0;
    ...
    >
      <div className="flex flex-col sm:flex-row items-center w-full gap-3">
        <div className="relative flex-1 w-full sm:w-auto">
          ...
          <Search
            className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 transition-colors duration-200 pointer-events-none ${isFocused ? "text-primary" : "text-slate-400"}`}
          />
          <Input
            id="blog-search"
          ...
          <div className="shrink-0 flex-1 sm:flex-none">
            ...
            >
              ...
              <SelectContent className="rounded-xl border-slate-200 dark:border-slate-800">
                ...
                <SelectItem value="published" className="cursor-pointer font-medium text-xs">
                  {t("published")}
                </SelectItem>
                <SelectItem value="draft" className="cursor-pointer font-medium text-xs">
                  {t("draft")}
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            <div className="flex items-center gap-1 shrink-0 ml-1">
              ...
              <Button
                ...
                onClick={() => {
                  setValue("");
                  onClearSelection?.();
                  router.push(`${BASE_PATH}?page=1&pageSize=${pageSize}`);
                }}
              >
                {tCommon("clear")}
              </Button>
