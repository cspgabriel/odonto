...
import { createExpense, getDepartmentsForExpense, getLabVendorsForExpense, getInventoryForExpense } from "@/lib/actions/expense-actions";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-2">
        ...
        <Input {...register("title")} placeholder={t("formTitlePlaceholder")} className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
        {errors.title && <p className="text-xs text-destructive">{errors.title.message}</p>}
      </div>
      <div className="space-y-2">
        <Label>{t("formDescription")}</Label>
        <Textarea {...register("description")} placeholder={t("formDescriptionPlaceholder")} className="min-h-[80px] rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
      </div>
      <div className="space-y-2">
        <Label>{t("formAmount")} *</Label>
        <Input type="number" step={0.01} min={0} {...register("amount")} placeholder="0.00" className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
      ...
      <div className="space-y-2">
        ...
        <Select value={watch("category")} onValueChange={(v) => setValue("category", v as FormData["category"])}>
          <SelectTrigger className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]"><SelectValue placeholder={t("formSelectCategory")} /></SelectTrigger>
          <SelectContent>{EXPENSE_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{t(CATEGORY_KEYS[c])}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div className="space-y-2">
        ...
        <Select value={watch("paymentMethod")} onValueChange={(v) => setValue("paymentMethod", v as FormData["paymentMethod"])}>
          <SelectTrigger className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]"><SelectValue placeholder={t("formSelectPaymentMethod")} /></SelectTrigger>
          <SelectContent>{EXPENSE_PAYMENT_METHODS.map((p) => <SelectItem key={p} value={p}>{t(PAYMENT_KEYS[p])}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div className="space-y-2">
        <Label>{t("formStatus")}</Label>
        <Select value={watch("status")} onValueChange={(v) => setValue("status", v as FormData["status"])}>
          <SelectTrigger className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]"><SelectValue /></SelectTrigger>
          <SelectContent>{EXPENSE_STATUSES.map((s) => <SelectItem key={s} value={s}>{t(STATUS_KEYS[s])}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <DatePicker date={date} onSelect={(d) => setValue("date", d ?? new Date())} placeholder={t("formDatePlaceholder")} className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60" />
        {errors.date && <p className="text-xs text-destructive">{errors.date.message}</p>}
      </div>
      <div className="space-y-2">
        <Label>{t("formVendorSupplier")}</Label>
        <Input {...register("vendor")} placeholder={t("formVendorSupplierPlaceholder")} className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
      </div>
      <div className="space-y-2">
        <Label>{t("formReceiptUrl")}</Label>
        <Input {...register("receiptUrl")} placeholder={t("formReceiptUrlPlaceholder")} className="h-10 rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
      </div>
      <div className="space-y-2">
        <Label>{t("formNotes")}</Label>
        <Textarea {...register("notes")} placeholder={t("formNotesPlaceholder")} className="min-h-[80px] rounded-lg border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-[#0B0B1E]" />
      </div>
      <div className="flex gap-3 pt-2">
        <Button type="button" variant="outline" onClick={() => onSuccess?.()}>{tCommon("cancel")}</Button>
        <Button type="submit" isLoading={isSubmitting}>{t("formCreateExpense")}</Button>
      </div>
    </form>
  );
}
